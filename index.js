...
function renderRoster(){
 ...
 box.innerHTML=activeProfiles().map(p=>`<label class="roster-item ${isAdmin()?"":"locked"}"><input type="checkbox" data-roster="${p.id}" ${selected.has(p.id)?"checked":""} ${isAdmin()?"":"disabled"}><div><strong>${esc(p.displayName)}</strong>${p.role==="admin"?"<small>Admin</small>":""}</div></label>`).join("");
